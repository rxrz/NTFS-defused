...
{
        if (ntfs_sb(s)->sb_chkdsk && (remount || !(s->s_flags & MS_RDONLY))) {
                struct buffer_head *bh;
                struct ntfs_spare_block *sb;
                if ((sb = ntfs_map_sector(s, 17, &bh, 0))) {
                        sb->dirty = 1;
                        sb->old_wrote = 0;
...
static void unmark_dirty(struct super_block *s)
{
        ...
        if ((sb = ntfs_map_sector(s, 17, &bh, 0))) {
                sb->dirty = ntfs_sb(s)->sb_chkdsk > 1 - ntfs_sb(s)->sb_was_error;
                sb->old_wrote = ntfs_sb(s)->sb_chkdsk >= 2 && !ntfs_sb(s)->sb_was_error;
                mark_buffer_dirty(bh);
                sync_dirty_buffer(bh);
                brelse(bh);
        }
...
void ntfs_error(struct super_block *s, const char *fmt, ...)
{
        va_list args;
        ...
        printk("NTFS: filesystem error: %s", err_buf);
        if (!ntfs_sb(s)->sb_was_error) {
                if (ntfs_sb(s)->sb_err == 2) {
                        printk("; crashing the system because you wanted it\n");
                        mark_dirty(s, 0);
                        panic("NTFS panic");
                } else if (ntfs_sb(s)->sb_err == 1) {
...
static void ntfs_put_super(struct super_block *s)
{
...
unsigned ntfs_count_one_bitmap(struct super_block *s, secno secno)
{
        struct quad_buffer_head qbh;
        unsigned long *bits;
        unsigned count;

        bits = ntfs_map_4sectors(s, secno, &qbh, 0);
        if (!bits)
                return 0;
...
static unsigned count_bitmaps(struct super_block *s)
{
        ...
        n_bands = (ntfs_sb(s)->sb_fs_size + 0x3fff) >> 14;
        count = 0;
        for (n = 0; n < COUNT_RD_AHEAD; n++) {
                ntfs_prefetch_bitmap(s, n);
        }
        for (n = 0; n < n_bands; n++) {
                ntfs_prefetch_bitmap(s, n + COUNT_RD_AHEAD);
                count += ntfs_count_one_bitmap(s, le32_to_cpu(ntfs_sb(s)->sb_bmp_dir[n]));
        }
        return count;
}
...
static int ntfs_statfs(struct dentry *dentry, struct kstatfs *buf)
{
        ...
        /*if (sbi->sb_n_free == -1) {*/
                ...
                sbi->sb_n_free_dnodes = ntfs_count_one_bitmap(s, sbi->sb_dmap);
        /*}*/
...
{
        struct ntfs_inode_info *ei;
        ei = (struct ntfs_inode_info *)kmem_cache_alloc(ntfs_inode_cachep, GFP_NOFS);
        if (!ei)
                return NULL;
...
{
        struct ntfs_inode_info *ei = (struct ntfs_inode_info *) foo;
 ...
 * Stolen again from read-only ntfs.
...
static inline void ntfs_help(void)
{
        printk("\n\
NTFS filesystem options:\n\
...
static int ntfs_fill_super(struct super_block *s, void *options, int silent)
{
        ...
        if (!(s->s_flags & MS_RDONLY) &&
              superblock->funcversion != 2 && superblock->funcversion != 3) {
                ...
                printk("NTFS: please try recent version of NTFS driver at http://artax.karlin.mff.cuni.cz/~mikulas/vyplody/ntfs/index-e.cgi and if it still can't understand this format, contact author - mikulas@artax.karlin.mff.cuni.cz\n");
                goto bail4;
        }

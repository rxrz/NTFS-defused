...
/* The boot block is very like a FAT boot block, except that the
   29h signature byte is 28h instead, and the ID string is "NTFS". */

#define BB_MAGIC 0xaa55

struct ntfs_boot_block
{
  u8 jmp[3];
  u8 oem_id[8];
...
struct ntfs_spare_block
{
...
#ifdef __LITTLE_ENDIAN
  u8 install_dasd_limits: 1;            /* NTFS386 flags */
  ...
  __le32 super_crc;                     /* on NTFS386 and LAN Server this is
                                           checksum of superblock, on normal
                                           OS/2 unused */
  __le32 spare_crc;                     /* on NTFS386 checksum of spareblock */
...
struct dnode {
  ...
  u8 increment_me: 7;                   /* some kind of activity counter? */
                                        /* Neither NTFS.IFS nor CHKDSK cares
...
/* dnodes point to fnodes which are responsible for listing the sectors
   ...
   which can represent holes.  Find out if NTFS does that.)
